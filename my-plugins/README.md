...
The tab list still needs a per-window answer, and it is derived rather than stored. `#{P:...}` loops the window's panes when the status bar is drawn, giving one glyph per live session in pane order — `🔓🔓🧠✅` is four sessions, the first two blocked on permission. An earlier version had the hook keep a window-level aggregate instead, which drifts: that value has an input the hook cannot observe, namely tmux moving a pane between windows, so a break, a maximize or a pane dying without `SessionEnd` all left the tab wrong with nothing to correct it. Deriving at render time removes the whole class. Repetition also carries the count without the priority order a deduplicated `🔓2🧠✅` would need; it is not a position handle, though, since a pane holding no session contributes no glyph — the pane border is what carries identity.
